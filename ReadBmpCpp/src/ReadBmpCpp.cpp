...
public:
	...
	bool Load(const char *Filename) {
		...
		TRACE("ColorTableSize " << ColorTableSize << ", H: " << m_BitmapHeader.Height << ", W: " << m_BitmapHeader.Width);
		//TRACE("CsType: " << m_BitmapHeader.CsType << ", colors imp: " << m_BitmapHeader.ClrImportant);
		...
		TRACE("BitCount    " << m_BitmapHeader.BitCount);
		TRACE("Compression " << m_BitmapHeader.Compression);

		TRACE("m_BitmapHeader.HeaderSize " << m_BitmapHeader.HeaderSize);
...
int main()
{
	...
	TRACE("----");
	CBitmap bmp2("3m.bmp");
	bmp2.Save("3mConverted.bmp", 24);

	CBitmap bmp2a("3mConverted.bmp");

	TRACE("----");
	CBitmap bmp3("1m2f.bmp");
	//bmp3.Save("cvt.bmp", 4);
	TRACE("----");
	//CBitmap bmp4("cvt.bmp");
	return 0;
}
